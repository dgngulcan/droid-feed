...
val MIGRATION_3_4: Migration = object : Migration(3, 4) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("CREATE  INDEX `index_rss_source_id` ON `$POST_TABLE_NAME` (`source_id`)")
    }
}
...
private fun addSourceID(database: SupportSQLiteDatabase) {
    database.run {
        ...
        execSQL("INSERT INTO source_temp (url, name, is_active) SELECT url, name, is_active FROM `$SOURCE_TABLE_NAME`;")
        execSQL("DROP TABLE IF EXISTS `$SOURCE_TABLE_NAME`;")
        execSQL("ALTER TABLE source_temp RENAME TO `$SOURCE_TABLE_NAME`;")
        ...
        execSQL("DROP TABLE IF EXISTS `$POST_TABLE_NAME`;")
        execSQL("ALTER TABLE post_temp RENAME TO `$POST_TABLE_NAME`;")
    }
}